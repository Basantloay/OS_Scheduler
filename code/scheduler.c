...
///////////////// Preemtive Highest Priority HPF Algorithm \\\\\\\\\\\\
...
void HPF()
{
	...
	while (1)
	{
		...
		{
			x = getClk();
			if ( (!isEmptyPriority(readyPriorityQueue) || (readyPriorityQueue->front->priorityofqueue.process.prriority ==  runningProcess.process.priority))
			{
				f3 = fopen("test.log", "a+");
